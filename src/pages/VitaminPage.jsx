...
import { ArrowLeft, AlertTriangle, CheckCircle, Users, Pill, Phone } from "lucide-react";
      ...
      <div className={`bg-gradient-to-br ${vitamin.gradient} py-16 px-4`}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-start gap-6">
            <span className="text-7xl">{vitamin.emoji}</span>
            <div>
              <h1 className="font-display text-5xl font-black text-white mb-2">{vitamin.name}</h1>
              <p className="text-white/80 text-xl">{vitamin.tagline}</p>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={stat.label} className="bg-white rounded-xl p-4 text-center shadow-sm border border-gray-100">
              ...
              <div className="font-bold text-gray-800 text-sm leading-tight">{stat.value}</div>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-display text-xl font-bold text-amber-800">Deficiency Symptoms</h3>
            </div>
            <ul className="space-y-2">
              {vitamin.deficiencySymptoms.map((s) => (
                <li key={s} className="flex items-start gap-2 text-amber-900 text-sm">
          ...
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-display text-xl font-bold text-red-800">Toxicity / Excess Signs</h3>
            </div>
            <ul className="space-y-2">
              {vitamin.toxicitySigns.map((s) => (
                <li key={s} className="flex items-start gap-2 text-red-900 text-sm">
        ...
        <Section icon={CheckCircle} title="🍽️ Best Food Sources">
          <div className="grid sm:grid-cols-2 gap-3">
            {vitamin.sources.map((s) => (
              <div key={s.food} className="flex items-center justify-between bg-green-50 rounded-xl p-3 border border-green-100">
                <span className="font-medium text-gray-700 text-sm">{s.food}</span>
                <span className="text-gray-500 text-xs bg-white px-2 py-1 rounded-lg border">{s.amount}</span>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section icon={Users} title="Who Is at Risk?">
          <div className="flex flex-wrap gap-2">
            {vitamin.atRiskGroups.map((g) => (
              <span key={g} className="bg-blue-50 text-blue-700 text-sm px-3 py-1.5 rounded-lg border border-blue-200">
                👤 {g}
              </span>
            ))}
          </div>
        ...
        <Section icon={Pill} title="Supplement Guidance">
          <p className="text-gray-700 leading-relaxed">{vitamin.supplementGuidance}</p>
        </Section>
        ...
        <div className="bg-gradient-to-r from-primary to-blue-600 rounded-2xl p-6 text-white">
          ...
          <p className="text-blue-100 leading-relaxed">{vitamin.whenToSeeDoctor}</p>
        </div>
        ...
        {vitamin.relatedMinerals.length > 0 && (
          <div className="bg-purple-50 border border-purple-200 rounded-2xl p-6">
            ...
            <ul className="space-y-2">
              {vitamin.relatedMinerals.map((m) => (
                <li key={m} className="flex items-start gap-2 text-purple-900 text-sm">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
          ...
          <ul className="space-y-1">
            {vitamin.references.map((r) => (
              <li key={r} className="text-gray-500 text-sm flex items-start gap-2">
        ...
        <div className="flex justify-between pt-4">
          <Link to="/home" className="inline-flex items-center gap-2 text-leaf hover:underline font-medium">
            <ArrowLeft size={16} /> All Vitamins
          </Link>
          <Link to="/diagnosis" className="inline-flex items-center gap-2 bg-leaf text-white px-5 py-2.5 rounded-xl font-medium hover:bg-green-600 transition-colors text-sm">
            Check My Symptoms →
          </Link>
        </div>
      </div>
