...
const resources = [
    { href: "https://www.who.int", label: "WHO Guidelines" },
    { href: "https://ods.od.nih.gov", label: "NIH Supplements" },
    { href: "https://www.mayoclinic.org", label: "Mayo Clinic" },
    { href: "https://pubmed.ncbi.nlm.nih.gov", label: "PubMed Research" },
];

export default function Footer() {
    return (
        <footer className="bg-deep text-gray-300 pt-10 pb-6 mt-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm">
                            {quickLinks.map((link) => (
                                <li key={link.to}>
                                    <Link to={link.to} className="hover:text-green-400 active:text-green-500 transition-colors py-0.5 inline-block">
                                        {link.label}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm">
                            {resources.map((item) => (
                                <li key={item.href}>
                                    <a href={item.href} target="_blank" rel="noopener noreferrer" className="hover:text-green-400 active:text-green-500 transition-colors py-0.5 inline-block">
                                        {item.label}
                                    </a>
                                </li>
                            ))}
                ...
                {/* Bottom Section - 2 on top, 1 centered below for mobile */}
                <div className="border-t border-gray-700 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">

                    {/* Top Row for Mobile (Copyright & Developer) */}
                    <div className="flex w-full sm:w-auto justify-between sm:justify-start items-center gap-2 sm:gap-4">
                        <p className="text-[10px] sm:text-xs text-gray-500">
                            2026 VitaGuide - MTI Pharmacy Faculty
                        </p>

                        <div className="hidden sm:block h-3 w-px bg-gray-700" />

                        <a
                            href="https://www.linkedin.com/in/abdelrhman-ahmed-fathy2004"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-[10px] sm:text-xs text-gray-400 hover:text-white flex items-center gap-1 transition-colors group"
                        >
                    ...
                    {/* Bottom Row for Mobile (Centered Heart Text) */}
                    <p className="text-[10px] sm:text-xs text-gray-500 flex items-center justify-center gap-1 w-full sm:w-auto">
                        Made with <Heart className="text-red-400 fill-red-400 w-3 h-3 mx-0.5" /> for public health education
                    </p>
